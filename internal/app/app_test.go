...
func TestNew(t *testing.T) {
	...
	if m.panes[2].ID() != pane.PaneWitness {
		t.Errorf("pane 2 should be Witness, got %d", m.panes[2].ID())
	}
	if m.activePane != 0 {
...
func TestTabKey(t *testing.T) {
	...
	m = newM.(Model)
	if m.activePane != 0 {
		t.Errorf("after tab wrap: activePane = %d, want 0", m.activePane)
	}
}
...
func TestShiftTabKey(t *testing.T) {
	...
	// Shift+tab wraps backward to last pane
...
func TestNumberKeys(t *testing.T) {
	...
	// Press "3" to go to witnesses pane
